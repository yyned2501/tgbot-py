...
@scheduler.scheduled_job("cron",hour=12, minute=10, id="mysql_backup")
async def mysql_backup(client: Client, message: Message):
    """
    自动mysql备份
    """
    # === 生成带时间戳的备份文件名 ===
        ...
        if DB_INFO["dbset"] == "mySQL":            
            ...
            try:
                with open(backup_path, "w", encoding="utf-8") as f_out:
                    ...
                    if result.returncode != 0:
                        ...
                        re_mess = await client.send_document(PT_GROUP_ID['BOT_MESSAGE_CHAT'],f"数据库备份失败: {result.stderr}")  
                ...
                re_mess = await client.send_document(PT_GROUP_ID['BOT_MESSAGE_CHAT'],f"✅ 数据库备份成功: {backup_path_gz}") 
            ...
            for file in BACKUP_DIR.glob("*.sql"):
                ...
                if (now - mtime).days > RETENTION_DAYS:
                    ...
                    re_mess = await client.send_document(PT_GROUP_ID['BOT_MESSAGE_CHAT'],f"🗑️ 删除过期备份: {file}")
                    file.unlink()
        else:
